...
import { answerQuestion, pickOption } from './ai-solver';
...
function stopableWait(ms: number, stopSignal: { stopped: boolean } | null): Promise<void> {
...
function makeAbortController(stopSignal: { stopped: boolean } | null): AbortController {
  ...
  if (stopSignal) {
    ...
    (controller as any)._clearWatches = () => clearInterval(check);
  }
  return controller;
}
...
interface RunAgentOptions {
  ...
  stopSignal?: { stopped: boolean } | null;
}
...
export async function runAgent(options: RunAgentOptions = {}): Promise<{ applied: number }> {
  const {
  ...
  const log = (msg: string, type = 'info') => emit({ type, message: msg, time: Date.now() });
  ...
  while (i < totalCards && appliedCount < maxApplications && !(stopSignal && stopSignal.stopped)) {
    ...
    try {
      ...
      log(`Job #${i}: Error - ${err instanceof Error ? err.message : String(err)}`, 'error');
    }
  }
...
async function findEasyApplyButton(page: Page): Promise<ElementHandle | null> {
...
async function openEasyApply(page: Page, log: (msg: string, type?: string) => void, stopSignal: { stopped: boolean } | null): Promise<boolean> {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    await btn.evaluate((el: Element) => (el as HTMLElement).click()).catch(() => {});
    await stopableWait(2500, stopSignal);
    if (await page.$(modalSel)) {
      return true;
    }
...
async function handleApplicationForm(
  ...
  while (steps < 20 && !(stopSignal && stopSignal.stopped)) {
    ...
    if (!modal) {
      ...
      try {
        ...
        const btnText = await page.evaluate(() => {
          const b = document.querySelector('button.jobs-apply-button, [data-test-id="apply-button"], .jobs-apply-button--top-card button') as HTMLElement | null;
          return b ? b.innerText.slice(0, 40) : 'none';
        }).catch(() => 'unknown');
        log(`  Apply button text now: ${btnText}`, 'warn');
      } catch (e) { log(`  Diag failed: ${e instanceof Error ? e.message : String(e)}`, 'warn'); }
      break;
    }
    ...
    const getProgress = async (): Promise<string | null> => {
      try {
        const bar = await page.$('progress[class*="completeness"], .artdeco-completeness-meter-linear__progress-element');
        return bar ? await bar.getAttribute('value') ?? String(await bar.evaluate((el: Element) => (el as HTMLProgressElement).value)) : null;
      } catch { return null; }
    };

    const dumpModal = async (why: string): Promise<void> => {
      const diag = await page.evaluate(() => {
        const modal = document.querySelector('.jobs-easy-apply-modal, [role="dialog"]') as HTMLElement | null;
        if (!modal) return { fields: [], errText: 'no modal text' };
        const fields = Array.from(modal.querySelectorAll('input, select, textarea, [contenteditable="true"]'))
          .filter((el: Element) => (el as HTMLInputElement).type !== 'hidden' && getComputedStyle(el).display !== 'none')
          .slice(0, 15)
          .map((el: Element) => {
            const wrap = el.closest('.fb-dash-form-element, .jobs-easy-apply-form-element, [class*="form-element"]') as HTMLElement | null;
            ...
            return {
              label: label || '',
              tag: el.tagName,
              type: (el as HTMLInputElement).type || el.getAttribute('contenteditable') || '',
              value: ((el as HTMLInputElement).value ?? (el as HTMLElement).innerText ?? '').trim().slice(0, 60),
              disabled: !!(el as HTMLInputElement).disabled,
              err: hasErr,
            };
          });
        const msg = modal.querySelector('.artdeco-inline-feedback, [class*="error"][class*="form"]') as HTMLElement | null;
    ...
    const enabled = async (btn: ElementHandle | null): Promise<boolean> => {
      if (!btn) return false;
    ...
    if (submitBtn && await enabled(submitBtn)) {
      ...
      await submitBtn.click().catch(async () => submitBtn.evaluate((el: Element) => (el as HTMLElement).click()));
      ...
      log('Clicking Review...');
      await reviewBtn.click().catch(async () => reviewBtn.evaluate((el: Element) => (el as HTMLElement).click()));
      if (stopSignal && stopSignal.stopped) { await page.keyboard.press('Escape').catch(() => {}); break; }
      await waitFor(2000);
      ...
      log('Clicking Next...');
      await nextBtn.click().catch(async () => nextBtn.evaluate((el: Element) => (el as HTMLElement).click()));
      if (stopSignal && stopSignal.stopped) { await page.keyboard.press('Escape').catch(() => {}); break; }
      await waitFor(2000);
      const progressAfter = await getProgress();
      if (progressAfter && progressBefore && progressAfter === progressBefore) {
      ...
      if (anySubmit && await enabled(anySubmit)) {
        log('Submitting application...', 'success');
        await anySubmit.click().catch(async () => anySubmit.evaluate((el: Element) => (el as HTMLElement).click()));
        await waitFor(3000);
        submitted = true;
        break;
      }
...
async function fillVisibleQuestions(
  ...
  try {
    ...
    for (const field of fields) {
      ...
      const tag = await field.evaluate((el: Element) => el.tagName);
      const label = await field.evaluate((el: Element) => {
        const direct = el.getAttribute('aria-label') || el.getAttribute('placeholder') || '';
        if (direct) return direct;
        let wrap = el.closest('.jobs-easy-apply-form-element, .fb-dash-form-element, [class*="form-element"]') as HTMLElement | null;
        if (wrap) {
          const lbl = wrap.querySelector('label') as HTMLElement | null;
          if (lbl && lbl.innerText.trim()) return lbl.innerText.trim();
          const q = wrap.querySelector('[class*="label"], [class*="question"], [class*="title"], [class*="form-element__label"]') as HTMLElement | null;
        ...
        const row = el.closest('div') as HTMLElement | null;
        if (row) {
      ...
      if (tag === 'SELECT') {
        try {
          const currentValue = await field.evaluate(el => (el as HTMLSelectElement).value);
          const selText = await field.evaluate(el => (el as HTMLSelectElement).options[(el as HTMLSelectElement).selectedIndex]?.text || '');
          ...
          const currentSelectedText = await field.evaluate(el => (el as HTMLSelectElement).options[(el as HTMLSelectElement).selectedIndex]?.text || '');
          ...
          if (options.length === 1) {
            ...
            try {
              ...
              log(`Option pick failed for "${question}": ${e instanceof Error ? e.message : String(e)}`, 'error');
            }
          ...
          log(`Could not select dropdown "${label}": ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        continue;
      }

      const alreadyFilled = await field.evaluate(el => (el as HTMLInputElement).value && (el as HTMLInputElement).value.trim().length > 0);
      ...
      if (type === 'checkbox' || type === 'radio') {
        ...
        const checked = await field.evaluate(el => (el as HTMLInputElement).checked);
        if (checked) continue;
        ...
        const sameGroup: ElementHandle[] = [];
        for (const g of group) {
          const gName = await g.evaluate(el => (el as HTMLInputElement).name || '');
          if (gName && gName === name) sameGroup.push(g);
        }
        if (sameGroup.length === 0) continue;
        const optionLabels: string[] = [];
        for (const g of sameGroup) {
          const gLabel = await g.evaluate((el: Element) => {
            const wrap = (el as HTMLElement).closest('label, .fb-dash-form-element__radio-wrap, .fb-dash-form-element, [class*="radio"]') as HTMLElement | null;
            return wrap ? wrap.innerText.trim().slice(0, 40) : ((el as HTMLInputElement).value || '');
          });
          optionLabels.push(gLabel || (g as unknown as HTMLInputElement).value || '');
        }
        ...
        try {
          ...
          log(`Radio pick failed for "${question}": ${e instanceof Error ? e.message : String(e)}`, 'error');
        }
        let chosen = picked
          ? sameGroup.find((g, idx) => optionLabels.some((l, li) => li === idx && l.toLowerCase() === picked.toLowerCase()))
          : null;
        if (!chosen) chosen = optionLabels.some(l => /^yes$/i.test(l))
          ? sameGroup.find((g, idx) => /^yes$/i.test(optionLabels[idx]))
          : sameGroup[0];
        if (chosen) {
          await chosen.evaluate((el: Element) => (el as HTMLElement).click()).catch(() => {});
      ...
      try {
        ...
        log(`Could not fill "${label}": ${e instanceof Error ? e.message : String(e)}`, 'error');
      }
    }
  } finally {
    if ((abortController as any)._clearWatches) (abortController as any)._clearWatches();
  }
}
