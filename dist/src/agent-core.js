...
exports.loadResume = loadResume;
exports.runAgent = runAgent;
require("dotenv/config");
const playwright_1 = require("playwright");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const ai_solver_1 = require("./ai-solver");
...
async function runAgent(options = {}) {
    const { keywords = 'Full Stack Engineer', remote = true, maxApplications = 10, } = options;
    ...
    const context = await playwright_1.chromium.launchPersistentContext(AGENT_DATA, {
        headless: false,
        slowMo: 100,
...
async function findEasyApplyButton(page) {
    ...
    if (labelBtn && await labelBtn.isVisible().catch(() => false))
        return labelBtn;
    return null;
}
    ...
    while (steps < 20 && !(stopSignal && stopSignal.stopped)) {
        ...
        if (!modal) {
            ...
            try {
                ...
                log(`  Diag failed: ${e instanceof Error ? e.message : String(e)}`, 'warn');
            }
            break;
        }
        ...
        const getProgress = async () => {
            try {
                ...
                return bar ? await bar.getAttribute('value') ?? String(await bar.evaluate((el) => el.value)) : null;
            }
            catch {
                return null;
            }
        ...
        const dumpModal = async (why) => {
            const diag = await page.evaluate(() => {
                ...
                if (!modal)
                    return { fields: [], errText: 'no modal text' };
...
async function fillVisibleQuestions(page, userResumeText, log, stopSignal) {
    ...
    try {
        ...
        for (const field of fields) {
            ...
            const label = await field.evaluate((el) => {
                ...
                if (wrap) {
                    ...
                    if (q && q.innerText.trim())
                        return q.innerText.trim();
                }
                const parentLabel = el.closest('label');
                if (parentLabel && parentLabel.innerText.trim())
                    return parentLabel.innerText.trim();
                const prev = el.previousElementSibling;
                if (prev && prev.innerText && prev.innerText.trim() && prev.tagName !== 'SELECT')
                    return prev.innerText.trim().slice(0, 80);
            ...
            if (tag === 'SELECT') {
                try {
                    ...
                    const options = await field.evaluate((el) => Array.from(el.options)
                    ...
                    if (options.length === 1) {
                        ...
                        let picked = null;
                        try {
                            picked = await (0, ai_solver_1.pickOption)(question, optionList, abortController.signal);
                        }
                        catch (e) {
            ...
            if (type === 'checkbox' || type === 'radio') {
                ...
                let picked = null;
                try {
                    picked = await (0, ai_solver_1.pickOption)(question, optionLabels, abortController.signal);
                }
                catch (e) {
                ...
                let chosen = picked
                    ...
                    : null;
                if (!chosen)
                    chosen = optionLabels.some(l => /^yes$/i.test(l))
            ...
            try {
                let answer = await (0, ai_solver_1.answerQuestion)(label, userResumeText, abortController.signal);
        ...
        if (abortController._clearWatches)
            abortController._clearWatches();
    }
}
